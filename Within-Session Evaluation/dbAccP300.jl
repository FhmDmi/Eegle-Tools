...
clf = MDM # select classifier, only MDM and ENLR are currently supported for P300 
# Standard machine learning pipeline using Eegle and saving results using DelimitedFiles
for (db, DB) ∈ enumerate(DBs)
    println("Database: ", (DB.dbName, DB.condition))
