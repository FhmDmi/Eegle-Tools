...
# peformers : if res.p < 0.005 ==> good performer :g
# if 0.005 < res.p < 0.05 ==> normal performer ? :n
# if res.p > 0.05 ==> bad performer :b

clf = MDM # select classifier, only MDM, ENLR and SVM
for (db, DB) ∈ enumerate(DBs)
    println("Database: ", (DB.dbName, DB.condition))
